...
double FEM<dim>::basis_gradient(unsigned int node, double xi){
  ...
  for(unsigned int i =0; i < (basisFunctionOrder+1); i++){
    ...
    if(i != node)
      {
	for(unsigned int j = 0; j < (basisFunctionOrder+1); j++)
  	  {
  	    if(j != i && j != node){
  	      partial *= (xi - xi_at_node(j))/(xi_at_node(node)-xi_at_node(j));
  	    }
  	    else if(j == i && j != node)
  	      {
  		partial *= 1./(xi_at_node(node)-xi_at_node(j));
  	      }
  	  }
  	value += partial;
      }
...
void FEM<dim>::assemble_system(){
  ...
  for (;elem!=endc; ++elem){
	...
	//EDIT - Define Flocal.
	Flocal[A] += quad_weight[q]*Area*h_e/2.*Force*x*basis_function(A,quad_points[q]);
	//Flocal[A] += Area * h_e * 0.5 * basis_function(A, quad_points[q]) * 
	//(Force * x) * quad_weight[q];
	...
	for(unsigned int q=0; q<quadRule; q++){
	  //EDIT - Define Klocal.
	  Klocal[A][B] += E*Area*2./h_e * basis_gradient(A,quad_points[q]) * basis_gradient(B,quad_points[q])*quad_weight[q];
	}
      }
    }
...
double FEM<dim>::l2norm_of_error(){
  ...
  for (;elem!=endc; ++elem){
	...
	u_h += D[local_dof_indices[B]]*basis_function(B,quad_points[q]);
      ...
      if(prob ==1)
      	{
	  double c_1 = (E*Area*(g2-g1)+Force*Area*pow(L,3)/6)/L;
	  double c_2 = E*Area*g1;
	  u_e = (- Force*Area*pow(x,3)/6. + c_1*x + c_2)/(E*Area);
      }
      else
      {
	double c_1 = NBC + Force * Area * L *L /2.;
	double c_2 = g1*E*Area;
        u_e =( - Force*Area*pow(x,3)/6. + c_1*x + c_2)/(E*Area);
      }

      l2norm += h_e /2. * quad_weight[q] *(u_h-u_e)*(u_h-u_e);
    }
  }
