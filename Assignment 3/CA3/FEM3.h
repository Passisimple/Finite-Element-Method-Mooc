  ...
  for (;elem!=endc; ++elem){
	...
	for(unsigned int i=0; i<dim; i++){ //Loop over nodal dofs
	  for (unsigned int B=0; B<nodes_per_elem; B++) {
        ...
        Klocal[dim*A+i][dim*B+k] += C(i,j,k,l) * fe_values.JxW(q) * fe_values.shape_grad(dim*A+i,q)[j]*  fe_values.shape_grad(dim*B+k,q)[l];
		}
	      }
	    }
	  }
    ...
    h[2] =  1.e9*x;
	  for (unsigned int A=0; A<nodes_per_elem; A++){ //loop over all element nodes
        ...
        Flocal[dim*A+i] += fe_face_values.JxW(q) * fe_face_values.shape_value(dim*A+i,q) * h[i];
	    }
	  }
	}
      }
