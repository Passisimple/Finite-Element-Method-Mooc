...
// Helper function to Find xi at the given node
template <int dim>
...
void FEM<dim>::define_boundary_conds(){
  ...
  for(unsigned int i = 0 ; i < totalNodes ; i++)
  {
    ...
    {
      // Apply B.C to top and bottom y coordinate, if exists same procedure can be applied to x as well.
      if(j == 1 && nodeLocation[i][j] == 0.)
        boundary_values[i] = 300.*(1. + 1./3.*(nodeLocation[i][0]));
      else if (j == 1 && nodeLocation[i][j] == 0.08)
	    ...
	    for(unsigned int i=0;i<dim;i++){
          ...
          for(unsigned int I=0;I<dim;I++){
            for(unsigned int J=0;J<dim;J++){
              ...
              Klocal[A][B] +=  detJ*basis_gradient(A,quad_points[q1],quad_points[q2])[i]*invJacob[i][I]
              *basis_gradient(B,quad_points[q1],quad_points[q2])[j]*invJacob[j][J]*(kappa[I][J])*quad_weight[q1]*quad_weight[q2]	;
            }
		      }
	      }
	    }
