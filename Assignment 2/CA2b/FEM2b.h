	...
	for(unsigned int q3=0; q3<quadRule; q3++){
	  ...
	  for(unsigned int A=0; A<dofs_per_elem; A++){
		...
		for(unsigned int j=0;j<dim;j++){
          ...
          Klocal[A][B] +=  detJ*basis_gradient(A,quad_points[q1],quad_points[q2],quad_points[q3])[i]*invJacob[i][I]
          *basis_gradient(B,quad_points[q1],quad_points[q2],quad_points[q3])[j]*invJacob[j][J]
          *(kappa[I][J])*quad_weight[q1]*quad_weight[q2]*quad_weight[q3]	;
