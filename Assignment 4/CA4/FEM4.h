...
void FEM<dim>::define_boundary_conds(){
  ...
  {
    if(almostEqual(nodeLocation[i][0],0.)) 
    {boundary_values_of_D[i] = 300.; boundary_values_of_V[i] = 0.;}
    else if(almostEqual(nodeLocation[i][0],1.)) 
    {boundary_values_of_D[i] = 310.; boundary_values_of_V[i] = 0.;}
  }

}

// Checks whether the two given double values are essentially equal in the specified tolerance.
template <int dim>
bool FEM<dim>::almostEqual(double a , double b)
{
  double tol = 1e-8;
  if(fabs(a-b) > tol)
  {
	...
	for(unsigned int q=0; q<num_quad_pts; q++){
	  for(unsigned int i=0; i<dim; i++){
	    for(unsigned int j=0; j<dim; j++){
	      ...
	      Klocal[A][B] += fe_values.JxW(q) * kappa[i][j] * fe_values.shape_grad(A,q)[i] * fe_values.shape_grad(B,q)[j];
	    }
	  }
	}
      }
